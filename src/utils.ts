...
function buildScreenHref(platform: string, screenName: string, domain?: string): string {
...
function buildWebPageHref(domain: string, path: string): string {
  const normalizedPath = path.replace(/^\/+/, "").trim();
  const pathPart = normalizedPath ? `/${normalizedPath}` : "";
  const protocol = domain === "localhost" || domain.startsWith("127.") ? "http" : "https";
  return `${protocol}://${domain}${pathPart}`;
}
function buildUserAgent(sdkVersion: string, platform: string, osVersion: string, deviceModel: string): string {
  if (platform === "ios") {
...
function sanitizeCustomProperties(properties: Record<string, any>): Record<string, string> | null {
...
function isSessionExpired(sessionStartTime: number): boolean {
  const SESSION_TTL_MS = 30 * 60 * 1e3;
  return Date.now() - sessionStartTime > SESSION_TTL_MS;
}
var AD_CLICK_PARAM_KEYS = [
  "gclid",
  "gclsrc",
  "wbraid",
  "gbraid",
...
function extractAdClickIds(href: string): Record<string, string> | undefined {
  if (!href || typeof href !== "string") return void 0;
  try {
