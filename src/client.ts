...
const { AnalyticsConfig, TrackEventOptions, TrackPageviewOptions, TrackIdentifyOptions, TrackPaymentOptions } = require("./types");
...
function computeOrigin(config: any) {
  ...
  const isLocalhost = domain === "localhost" || domain.startsWith("127.") || domain.startsWith("192.168.") || domain.startsWith("10.") || domain.startsWith("::1");
...
var DataFastClient = class {
  ...
  constructor() {
    ...
    this.queue = null;
    this.logger = utils.createLogger(false);
  }
  /**
  ...
  async init(config: any) {
    ...
    this.logger = utils.createLogger(config.debug ?? false);
    ...
    const userAgent = utils.buildUserAgent(
      SDK_VERSION,
      config.platform,
   ...
   * @param options.href - Full URL override. When provided (web), used as-is instead of constructing from domain + screenName.
   */
  async trackScreen(screenName: string, options?: { href?: string }) {
    ...
    const DEDUP_MS = 6e4;
    if (this.state.lastPageviewUrl === payload.href && this.state.lastPageviewTime && Date.now() - this.state.lastPageviewTime < DEDUP_MS) {
      this.logger.log("Pageview throttled (same URL within 60s):", payload.href);
      return;
    }
    let referrer = null;
    if (this.state.lastScreenName) {
      referrer = config.platform === "web" ? buildWebPageHref(config.domain, this.state.lastScreenName) : buildScreenHref(config.platform, this.state.lastScreenName, config.domain);
    } else if (config.platform === "web" && typeof document !== "undefined" && document.referrer) {
      referrer = document.referrer;
    }
    (payload as any).referrer = referrer;
  ...
  async track(eventName: string, properties?: Record<string, any>) {
    if (!this.isReady()) return;
    if (!utils.isValidEventName(eventName)) {
      this.logger.warn(
    ...
    const sanitized = utils.sanitizeCustomProperties(properties ?? {});
    if (sanitized === null) {
      this.logger.warn("Custom properties validation failed");
      return;
    }
    const screenName = this.state.lastScreenName ?? "Unknown";
    const payload = this.buildBasePayload("custom", screenName, undefined);
    (payload as any).extraData = {
  ...
  async identify(userId: string, properties?: Record<string, any>) {
    ...
    const screenName = this.state.lastScreenName ?? "Unknown";
    const payload = this.buildBasePayload("identify", screenName, undefined);
    (payload as any).extraData = {
      user_id: userId,
  ...
  async trackPayment(data: any) {
    if (!this.isReady()) return;
    const hasIdentifier = typeof data.email === "string" && data.email.length > 0 || typeof data.stripe_session_id === "string" && data.stripe_session_id.length > 0 || typeof data.lemonsqueezy_order_id === "string" && data.lemonsqueezy_order_id.length > 0 || typeof data.polar_checkout_id === "string" && data.polar_checkout_id.length > 0 || typeof data.paddle_transaction_id === "string" && data.paddle_transaction_id.length > 0;
    if (!hasIdentifier) {
      this.logger.warn(
    ...
    const screenName = this.state.lastScreenName ?? "Unknown";
    const payload = this.buildBasePayload("payment", screenName, undefined);
    (payload as any).extraData = {
  ...
  async trackExternalLink(url: string, text?: string) {
    ...
    const screenName = this.state.lastScreenName ?? "Unknown";
    const payload = this.buildBasePayload("external_link", screenName, undefined);
    (payload as any).extraData = {
      url,
      ...text && { text }
    };
  ...
  /**
   * Whether the client was initialized in cookieless (web) mode
   */
  isCookieless() {
    return !!this.state.config?.cookieless;
  }
  /**
   * Shutdown the SDK
   */
  ...
  async onEventSendSuccess(parsedBody: any) {
  ...
  buildBasePayload(type: string, screenName: string, hrefOverride?: string) {
    const config = this.state.config;
    const deviceInfo = this.state.deviceInfo;
    const href = hrefOverride ?? (config.platform === "web" ? buildWebPageHref(config.domain, screenName) : buildScreenHref(config.platform, screenName, config.domain));
    const adClickIds = config.platform === "web" ? extractAdClickIds(href) : void 0;
    const payload = {
      websiteId: config.appId,
      domain: config.domain,
      visitorId: config.cookieless ? generateVisitorId() : this.state.visitorId,
    ...
    if (config.cookieless) {
      (payload as any).cookieless = true;
    }
    if (adClickIds) {
      (payload as any).adClickIds = adClickIds;
    }
    return payload;
  }
};
